...
function SaveFileInResource(user_id, data, inf) {
    ...
    try {
        sCheckResult = tools.check_resource_size(StrLen(oFileField), user_id);
        sName = inf != undefined ? inf.name : oFileField.FileName;
        ...
        docResource.TopElem.put_str(oFileField, UrlFileName(sName));
        docResource.Save();
            ...
            SELECT \
                ( \
                    ...
                    FROM \
                        ...
                        INNER JOIN lector l ON l.id = ls.id \
                    WHERE \
    ...
    try {
        ...
        for (oNewListCollaboratorEvent in aNewListCollaboratorEvent) {
            if (ArrayOptFindBySortedKey(aCurCollaboratorEvent, OptInt(oNewListCollaboratorEvent.id, 0), "id") == undefined) {
                // tools.add_person_to_event(OptInt(oNewListCollaboratorEvent.id, 0), eventDoc.DocID, null, eventDoc);
    ...
    try {
        ...
        if (oFile != null && oFile.FileName.HasValue) {
            ...
            if (oFileID.error == 0) {
                ...
                try {
                    strSuffix = UrlPathSuffix(excelFileUrl);
                    if (strSuffix == ".xls" || strSuffix == ".xlsx") {
                        aRows = OpenDoc(excelFileUrl, "format=excel").TopElem.worksheet;
                    } else if (strSuffix == '.csv') {
                        aRows = OpenDoc(excelFileUrl, "format=csv;delim=sem").TopElem;
                    } else {
                        aRows = OpenDoc(excelFileUrl, "format=csv;delim=tab").TopElem;
                    }
                } catch (e) {
                    alert("При загрузке произошла ошибка: " + e);
                }

                for (oRow in aRows) {
                    sCode = String(Trim(oRow[0]));
                    sFullname = String(Trim(oRow[1]));
...
function GetInfoForCreateEvent(user_id, httpRequest) {
    try {
        ...
        return EncodeJson({ success: true, error: "", data: aActivites, sample_id: "/download_file.html?file_id=" + String(iFileID) });
    } catch (e) {
        ...
        sQuery = "sql: \
            ...
            SELECT \
                ...
                CASE \
                    ...
                    WHEN e.data.value('event/max_person_num', 'int') IS NOT NULL AND ISNULL(e.data.value('(event/max_person_num)[1]', 'int'), 0) <= (SELECT COUNT(1) FROM event_results ers WHERE ers.event_id = es.id AND ers.not_participate = 0) THEN 'нет мест' \
                    ELSE 'записаться' \
                END status \
            FROM \
                events es \
...
function ExportListCollaboratorsFromEvent(user_id, teUser, httpRequest) {
    try {
        ...
        if (oEvent != undefined) {
            var aHeaderExcel = [
                { title: "Название отчета", value: 'Список сотрудников по мероприятию "' + oEvent.event_name + '"', type: 'single' },
                { title: "Дата формирования отчета", value: StrDate(Date(), false, false), type: 'single' }
            ];

            var aTitleExcel = [
                { name: "person_fullname", title: "ФИО сотрудника", width: 30, res_null: "", type: "str" },
                { name: "person_position_name", title: "Подразделение", width: 50, res_null: "", type: "str" },
                { name: "assist", title: "Присутствие", width: 25, res_null: "", type: "str" },
                { name: "not_participate", title: "Отказался от участия", width: 25, res_null: "", type: "str" },
                { name: "comment", title: "Комментарий", width: 25, res_null: "", type: "str" }
            ];

            sUrl = createExcel(aData, aTitleExcel, aHeaderExcel, user_id, teUser);
...
function createExcel(arrReport, aTitleExcel, aHeaderExcel, curUserID, curUser) {
    function cellSetStyle(oCellParam, sStyleParam) {
        if (sStyleParam == "title") {
            oCellParam.Style.IsBold = true;
            oCellParam.Style.HorizontalAlignment = "Center";
            oCellParam.Style.VerticalAlignment = "Center";
            oCellParam.Style.IsTextWrapped = true;
            oCellParam.Style.Borders.SetColor("black");
            oCellParam.Style.Borders.SetStyle("Thin");
        }
        else if (sStyleParam == "td_left_border") {
            oCellParam.Style.IsBold = false;
            oCellParam.Style.HorizontalAlignment = "left";
            oCellParam.Style.VerticalAlignment = "Center";
            oCellParam.Style.IsTextWrapped = true;
            oCellParam.Style.Borders.SetColor("black");
            oCellParam.Style.Borders.SetStyle("Thin");
    ...
    function getExcelLetter(countColl) {
    ...
    function setExcelTitle(aHeaderExcel) {
        for (oHeaderExcel in aHeaderExcel) {
            switch (oHeaderExcel.type) {
                case 'single':
                    oCell = oWorksheet.Cells.GetCell(getExcelLetter(1) + iRow);
                    oCell.Value = oHeaderExcel.title;
                    oWorksheet.Cells.Merge(iRow - 1, 1, 1, 2);
                    oCell = oWorksheet.Cells.GetCell(getExcelLetter(2) + iRow);
                    oCell.Value = oHeaderExcel.value;
                    iRow++;
                    break;
                case 'multi':
                    if (ArrayOptFirstElem(oHeaderExcel.value) != undefined) {
                        iHeight = 13;
                        if (ArrayOptFirstElem(oHeaderExcel.value) != undefined) {
                            iHeight = 13 * ArrayCount(oHeaderExcel.value);
                        }
                        if (iHeight > 130) {
                            iHeight = 130;
                        }
                        oWorksheet.Cells.SetRowHeight(iRow - 1, iHeight);
                        oCell = oWorksheet.Cells.GetCell(getExcelLetter(1) + iRow);
                        oCell.Value = oHeaderExcel.title;
                        oCell.Style.VerticalAlignment = "Top";

                        oWorksheet.Cells.Merge(iRow - 1, 1, 1, 5);
                        oCell = oWorksheet.Cells.GetCell(getExcelLetter(2) + iRow);
                        oCell.HtmlString = getElemName(oHeaderExcel.value, "subdivision", "name");
                ...
                case 'bit':
                    ...
                    oWorksheet.Cells.Merge(iRow - 1, 1, 1, 2);
                    oCell = oWorksheet.Cells.GetCell(getExcelLetter(2) + iRow);
                    oCell.Value = oHeaderExcel.value ? "Да" : "Нет";
                    iRow++;
                    break;
                default:
                    break;
        ...
        iRow++;
        iColumn = 1;
        for (oTitleExcel in aTitleExcel) {
            oWorksheet.Cells.SetColumnWidth(iColumn - 1, oTitleExcel.width)
            oCell = oWorksheet.Cells.GetCell(getExcelLetter(iColumn) + iRow);
            oCell.Value = oTitleExcel.title;
            cellSetStyle(oCell, "title");
            iColumn++;
        }
    }

    function setExcelBody(arrReport, aTitleExcel) {
        var iCountReport = 0;
        for (oReportElem in arrReport) {
            iRow++;
            iCountReport++;
            iColumn = 1;
            ...
            for (oTitleExcel in aTitleExcel) {
                oCell = oWorksheet.Cells.GetCell(getExcelLetter(iColumn) + iRow);
                sValue = oReportElem.GetOptProperty(oTitleExcel.name, "");
                if (sValue == "") {
                    sValue = oTitleExcel.GetOptProperty("res_null", "");
                }

                if (sValue == "") {
                    oCell.Value = null;
                } else {
                    if (oTitleExcel.GetOptProperty("type", "") == "int") {
                        oCell.Style.Number = 1;
                        oCell.Value = OptInt(sValue, null);
                    } else if (oTitleExcel.GetOptProperty("type", "") == "date") {
                        oCell.Style.Number = 14;
                        oCell.Value = sValue != null ? Date(sValue) : null;
                    } else {
                        oCell.Value = sValue;
                    }
                }

                cellSetStyle(oCell, "td_left_border");
                iColumn++;
            }
        }
    }
    ...
    var iRow = 1;
    var iHeight;
    sFile_name = ObtainTempFile(".xlsx");
    oExcelDoc = new ActiveXObject("Websoft.Office.Excel.Document");
    oExcelDoc.CreateWorkBook();
    oWorksheet = oExcelDoc.GetWorksheet(0);
    oExcelDoc.Open(sFile_name);

    setExcelTitle(aHeaderExcel);
    setExcelBody(arrReport, aTitleExcel);
    oExcelDoc.SaveAs(UrlToFilePath(sFile_name));

    docResource = OpenNewDoc("x-local://wtv/wtv_resource.xmd");
    docResource.TopElem.code = "hc_list_collaborators_event";
    docResource.TopElem.person_id = curUserID;
    docResource.TopElem.type = "file";
    tools.common_filling("collaborator", docResource.TopElem, curUserID, curUser);
    docResource.BindToDb();

    sFileNameExcel = "hc_list_collaborators_event_" + StrReplace(StrDate(Date(), true, true), ":", "-") + ".xlsx";
    sFile_nameTemp = FileName(UrlToFilePath(sFile_name));
    sFile_nameDir = StrReplace(sFile_name, sFile_nameTemp, "");
    sFile_nameNew = UrlAppendPath(sFile_nameDir, sFileNameExcel);
    MoveFile(UrlToFilePath(sFile_name), UrlToFilePath(sFile_nameNew));

    docResource.TopElem.put_data(sFile_nameNew);
    docResource.Save();

    DeleteFile(sFile_nameNew);

    return "/download_file.html?file_id=" + docResource.DocID;
}
...
function AssistCollaboratorsEvent(user_id, httpRequest) {
    try {
        ...
        for (oEventResult in aEventResults) {
            ...
            if (oNewAssist != undefined && oEventResult.marked != bAssist) {
